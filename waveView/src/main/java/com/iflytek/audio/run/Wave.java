...
public class Wave {
    ...
    public Wave(WaveView waveView, int offset) {
        float[] dat = new float[2160 + offset];
        new Thread(() -> {
            for (int i = 0; i < 2160 + offset; i++) {
                dat[i] = (Sine12bit[i % 256] - 2048) / 256;
            }
            sleep(100);
            while (true) {
                sleep(50);
                waveView.showLines(dat);
            }
        }).start();
    }
}
